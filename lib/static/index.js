...
const render = renderer => (Component, props) => renderer(
...
const renderHTML = (Root, Component, options) => {
  ...
  const renderer = isStatic ? renderToStaticMarkup : renderToString
  const body = render(renderer)(Component, options)
  ...
  const html = Root
    ? render(renderer)(Root, rootProps)
    : render(renderer)(Html, rootProps)
